...
const Swap: React.FC = () => {
  ...
  const onChangeToken1 = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (isValid(t)) {
      ...
      let value: string = e.target.value
      setToken1(value)
      const res =  swappingUI  ? await getOtherTokenPrice(value, token0Address, token1Address) : await getOtherTokenPrice(value, token1Address, token0Address)
  ...
  return (
    <Card title="Swap" rightComponent={<SwapSetting />}>
      ...
      <Collapse
        header={
          <Text variants="h5">
            {!swappingUI ? `${token0InitalImpact}${ticker1} = ${token1InitialImpact}${ticker2}` : `${token1InitialImpact}${ticker2} = ${token0InitalImpact}${ticker1}`}
          </Text>
        }>
